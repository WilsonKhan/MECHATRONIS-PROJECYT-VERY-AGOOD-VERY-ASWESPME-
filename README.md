...
Game Flow
Host presses ARM button on the master console
Both contestant pods are armed and can now buzz in
First player to press their buzz button locks out the other player
Host reads the trivia question from the PowerPoint
Player answers verbally
Host presses CORRECT (point awarded) or WRONG (round re-arms for other player)
Scores update on pod LEDs and PC scoreboard
Every 3rd question: optional roulette bonus round with spinning wheel
...
Ensure CMSIS headers are in the Drivers/ folder (see below)
Project settings: add `../Drivers` to include paths, add `STM32F303xC` to preprocessor defines
Build: Project > Build All
Flash: Run > Run As > STM32 C/C++ Application
...
Open pod_firmware.ino in Arduino IDE
Set MY_POD_ID to 0x01 for Pod 1 or 0x02 for Pod 2
